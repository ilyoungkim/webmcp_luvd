...
with tab4:
    ...
    if not tenants:
        ...
        with st.form(key=f"tenant_form_{sel['id']}"):
            ...
            with col_e:
                # ── 프로바이더 선택 (gemini / openrouter / openai 등) ──
                PROVIDERS = ["gemini", "openrouter", "openai", "anthropic"]
                current_provider = (sel.get("provider") or "gemini").lower()
                new_provider = st.selectbox(
                    ...
                    key=f"provider_{sel['id']}",
                )

            # ── 모델 선택 (프로바이더별 사전 정의 목록 + 사용자 지정) ──
            #   gemini     : Gemini generateContent 모델
            #   openrouter : OpenRouter 모델 ID (예: openai/gpt-4o, anthropic/claude-3.5-sonnet)
            #   openai     : OpenAI 모델 (예: gpt-4o, gpt-4o-mini)
            #   anthropic  : Anthropic 모델 (예: claude-3-5-sonnet-20241022)
            MODEL_PRESETS = {
                "gemini": [
            ...
            presets = MODEL_PRESETS.get(new_provider, [])
            model_options = presets + ["✏️ 사용자 지정"]
            current_model = sel["model_name"]
            default_index = (
                presets.index(current_model)
                if current_model in presets
                else len(presets)  # "사용자 지정" 인덱스
            )
            new_model = st.selectbox(
                f"모델 (model_name) — {new_provider}",
        ...
        if submitted:
            ...
            if new_provider != (sel.get("provider") or "gemini"):
                db.set_tenant_provider(sel["id"], new_provider)
                updated = True
