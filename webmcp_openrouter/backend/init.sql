...
CREATE TABLE IF NOT EXISTS tenants (
    ...
    gemini_key    TEXT         NOT NULL,             -- LLM API 키 (Gemini/OpenRouter 등, DB에만 보관)
    password      VARCHAR(255) NULL,                 -- 개인별 대시보드 로그인 비밀번호 (선택)
    provider      VARCHAR(32)  NOT NULL DEFAULT 'gemini', -- 'gemini' | 'openrouter' | 'openai' ...
...
-- ── 4. (선택) 개발용 테넌트 시드 데이터 ─────────────────────────
-- 실제 키로 교체하세요. (운영 배포 시 주석 해제)
-- provider: 'gemini' | 'openrouter' | 'openai' ...
-- INSERT INTO tenants (origin, site_ns, gemini_key, provider, model_name, rate_limit, tier) VALUES
--   ('http://localhost:8000',      'yonja', 'YOUR_KEY', 'gemini',     'gemini-3.5-flash-lite', 100, 'dev'),
--   ('http://localhost:3000',      'yonja', 'YOUR_KEY', 'gemini',     'gemini-3.5-flash-lite', 100, 'dev'),
--   ('https://yonza.co.kr',        'yonja', 'YOUR_KEY', 'gemini',     'gemini-3.5-flash-lite', 20,  'prod'),
--   ('http://192.168.31.136:8081', 'yonja', 'YOUR_KEY', 'gemini',     'gemini-3.5-flash-lite', 100, 'dev'),
--   ('http://114.205.189.190:8081', 'hospital', 'YOUR_KEY', 'gemini',  'gemini-3.5-flash-lite', 20, 'prod'),
--   -- OpenRouter 예시 (model_name 은 OpenRouter 모델 ID)
--   ('https://example.com',        'site', 'sk-or-v1-...', 'openrouter', 'openai/gpt-4o', 20, 'prod');

-- ── 5. 기존 DB에 provider 컬럼 추가 (마이그레이션) ──────────────
-- 이미 tenants 테이블이 존재하는 기존 설치에 적용:
